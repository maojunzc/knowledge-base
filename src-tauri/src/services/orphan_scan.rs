...
//! - 第一层判定：目录的 `<note_id>` 不在 DB 笔记 id 集合中 → 整个目录是未引用
//!   （reason = notePurged，对应 note 已被永久删除但素材残留）
//! - 第二层判定：目录在 DB 中但目录里的文件未被 content / source_file_path 引用 → 文件级未引用
//!   （reason = unreferenced）
...
/// 单组未引用素材明细上限（防止 UI 卡死 / 过大 IPC 包体）
...
impl OrphanScanService {
    ...
    pub fn scan_all(db: &Database, app_data_dir: &Path) -> Result<OrphanAssetScan, AppError> {
        ...
        // 全部存在的 note_id 集合（含 trash，用于"目录未引用"判定）
...
fn scan_per_note_dir(
    ...
    for entry in std::fs::read_dir(root)? {
        ...
        match parsed_id {
            ...
            // 目录名是 note_id 但 DB 里没有 → 整目录未引用（笔记已 purge）
